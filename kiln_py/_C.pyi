...
class Model:
    ...
    def forward_hidden_states(self, tokens: npt.NDArray[np.int32]) -> List[npt.NDArray[np.float32]]: ...
...
class CudaModel:
    """Available only when Kiln is built with KILN_BUILD_CUDA=ON."""
    def __init__(self, model: Model) -> None: ...
    def forward(self, tokens: npt.NDArray[np.int32],
                start_pos: int = 0) -> npt.NDArray[np.float32]: ...
    def forward_cached(self, tokens: npt.NDArray[np.int32],
                       start_pos: int) -> npt.NDArray[np.float32]: ...
